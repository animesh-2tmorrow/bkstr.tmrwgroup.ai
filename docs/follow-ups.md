...
### 85. Assistant system prompt tuning for proactive context surfacing

Surfaced during Phase 5 Stream B live testing (2026-05-11). Stream B v1 ships with a deliberately minimal system prompt: *"You are bkstr's admin assistant... You have 5 read-only tools — use them to query the database, never speculate about state you haven't fetched. You cannot make changes. Be concise. When a query is ambiguous, ask one clarifying question instead of guessing."* The assistant answers questions correctly but doesn't proactively surface adjacent context an admin would typically also care about. Concrete example from live test: asked "how many books we have," the assistant correctly answered "6 books, all ACTIVE" — but didn't mention that all 6 are owned by the same user (the seed-corpus state from #75 / the ADMIN-as-seed-owner runbook entry), which is the more interesting fact for any admin reviewing the catalog. The admin had to ask the follow-up "and who owns them?" to surface the concentration.

Three tuning paths to consider when this becomes load-bearing:

- **(a) Expand the system prompt with "when answering questions about X, also consider Y" hints.** Lowest cost, highest specificity per hint. Risk: prompt grows long enough that the model starts ignoring earlier rules. Each hint should be load-bearing for a real observed pattern, not speculative.
- **(b) Add a single steering line: "You are looking at an early-stage platform; surface ownership concentration, stale data, and configuration anomalies proactively rather than waiting for the admin to ask the follow-up question."** Broad direction without per-question hints; lets the model use judgment. Risk: too vague to change behavior.
- **(c) Inject a brief platform context summary at conversation start.** Example: *"Currently single-tenant Stripe sandbox, 3 users (1 ADMIN, 2 SUBSCRIBER), 6 books in seed corpus all owned by ADMIN as temporary state per #75, 0 publishers, 0 PURCHASE grants outside the Docker Patterns test purchase."* Highest signal per token, but requires either a static summary string (becomes stale fast) or a one-shot summarizer query at conversation creation time (extra Bedrock call + persistence column on `assistant_conversations`).
